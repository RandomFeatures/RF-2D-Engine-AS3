package com.random.iso.items 
{
	...
	/**
	 * in game objects that are not animated 
	 * ...
	...
	public class StaticObject extends GameObject
	{
		...
		protected var m_StaticImage:FlxLoadSprite;
		...
		public function StaticObject(editMode:Boolean, game:GameObjectManager) 
		{
			m_Game = game;
			m_EditMode = editMode;
		...
		private function loadFacingProperties(xml:XMLList, action:String):FacingProp
		{
		...
		public function get StaticImage():FlxLoadSprite 
		{
		...
		private function onAnimationDoneLoading(e:Event):void {
			...
			m_StaticImage.loadExtGraphic(m_AnimationLoader.SpriteImage, m_FileName, false, true, false, m_Width, m_Height, true);
			m_Width = m_StaticImage.width;
			m_Height = m_StaticImage.height;
			...
			{
				case 4:
					m_StaticImage.addAnimation(m_NE_Default.Action, m_NE_Default.Frames, m_NE_Default.FPS, m_NE_Default.Looped);
					m_StaticImage.addAnimation(m_NW_Default.Action, m_NW_Default.Frames, m_NW_Default.FPS, m_NW_Default.Looped);
				case 2:
					m_StaticImage.addAnimation(m_SW_Default.Action, m_SW_Default.Frames, m_SW_Default.FPS, m_SW_Default.Looped);
				case 1:
					m_StaticImage.addAnimation(m_SE_Default.Action, m_SE_Default.Frames, m_SE_Default.FPS, m_SE_Default.Looped);
			}
			...
			if (m_NewItem)
				m_StaticImage.SetFilter(m_NewItemGlowEffect);
		...
		override public function setDir(dir:String):void
		{
			...
			if (m_CurrentDirection == IsoConstants.DIR_SE)
			{
				...
				m_Rows = m_SE_Default.Rows;
				m_Cols = m_SE_Default.Cols
				m_StaticImage.play(m_SE_Default.Action);
			...
			if ((m_CurrentDirection == IsoConstants.DIR_SW) && m_FacingCount > 1)
			{
				...
				m_StaticImage.play(m_SW_Default.Action);
			}
			...
			if ((m_CurrentDirection == IsoConstants.DIR_NW) && m_FacingCount > 2)
			{
				...
				m_StaticImage.play(m_NW_Default.Action);
			...
			if ((m_CurrentDirection == IsoConstants.DIR_NE) && m_FacingCount > 2)
			{
				...
				m_StaticImage.play(m_NE_Default.Action);
		...
		override public function getMouseOver(x:int, y:int):Boolean
		{
			...
			if (!m_StaticImage) return false;
			
			if (!m_MouseOver)
			{//the mouse was not over the image on the last frame
				if (x > m_StaticImage.x && x < (m_StaticImage.x + m_Width))
				{//mouse in within the bounding box width
					if (y > m_StaticImage.y && y < (m_StaticImage.y + m_Height))
					{//mouse is also in the bounding box heoght
						if (m_StaticImage.pixels.hitTest(new Point(m_StaticImage.x, m_StaticImage.y), 0, new Point(x, y)))
